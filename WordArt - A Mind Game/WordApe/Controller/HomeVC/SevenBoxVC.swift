...
class SevenBoxVC: UIViewController {
    ...
    @IBOutlet var viewRow6: [customTileView]!
    ...
    func setInitialUI() {
        ...
        self.arrViewRows.append(self.viewRow6)
        ...
        // Statistics
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(named: "statistic"), style: .plain, target: self, action: #selector(onClickStatistics))
...
extension SevenBoxVC {
    // Choose any random word from array
    func fetchRandomWord() {
        // Fetch word dictionary
        self.arrValidWords = DataOfFile.shared.getRandomWordsFromCSV(fileName: "seven-letter-words", fileType: "txt")
        
        // Fetch random word list
        self.arrRandomWords = DataOfFile.shared.getRandomWordsFromCSV(fileName: "seven-letter-words", fileType: "csv")
    ...
    func changeTileBackground() {
        ...
        if self.currentRow == 6 || self.isWordMatch {
    ...
    func setCurrentTile(text: String, viewRow: [customTileView], lblRow: [UILabel], key: UIButton) {
        ...
        if self.currentRow <= 7 {
            if self.currentTileIndex < 7 {
                // Set character at perticular index
                if self.currentTileIndex == 6 && lblRow[self.currentTileIndex].text != "" {
                  //If last tile of row, can't change last character
                    ...
                    if lblRow[self.currentTileIndex].text != "" && self.currentTileIndex != 6{
...
extension SevenBoxVC {
    ...
    @IBAction func onClickPlayAgain(_ sender: UIButton) {
        ...
        self.btnSubmit.isHidden = false
        let initialViewController = self.storyboard?.instantiateViewController(withIdentifier: "SevenBoxVC") as! SevenBoxVC
